# Finans — döviz kuru kaynağı ve otomatik tazeleme
...
`fin_fx_daily` tablosundaki her satır **T.C. Merkez Bankası günlük döviz kuru
...
ay künyesinde iki kaynak birden görünür (`fin_fx_monthly.sources`).
...
alınacak olan o günlük değerlerdir. `fin_fx_monthly` view'ı bu yüzden
`avg(eur_try / usd_try)` yazar. Fark küçüktür ve tam da bu yüzden sessizce
yanlış yazılabilir — `lib/finance/__tests__/fx.test.ts` sayıyla dondurur.
...
İkisi de **aynı çekirdeği** çağırır (`lib/finance/fx-refresh.ts`); iki yerde
...
## Ödenmiş ayın kuru DONAR

`fin_periods.eur_try_rate` bir ayın **ödeme kurudur** ve satırın kendinde
...
Yeni aylarda ekran **ortalamayı önerir**, kullanıcı onaylar, değer donar.
