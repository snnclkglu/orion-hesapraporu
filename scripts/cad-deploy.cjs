...
for(const name of ['service','contracts']) {
 const source=fs.readFileSync(`src/lib/cad/${name}.ts`,'utf8').replace('from "zod"','from "npm:zod@4.4.3"').replace('from "@/lib/roles"','from "./roles.js"').replace('from "./contracts"','from "./contracts.js"');
...
(async()=>{
 ...
 for(const name of ['index.js','service.js','contracts.js','roles.js'])form.append('file',new Blob([fs.readFileSync(path.join(root,name))],{type:'application/javascript'}),name);
