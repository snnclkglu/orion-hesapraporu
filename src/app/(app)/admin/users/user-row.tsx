...
export function UserRow({
  ...
  return (
    // DAR EKRANDA SATIR BİR IZGARADIR. Hücreler yan yana ~640px istiyor;
    // telefonda satırın kendisi ekranın iki katı oluyordu. `xl` altında satır
    ...
    <TableRow className="max-xl:grid max-xl:gap-3 max-xl:px-2 max-xl:py-3">
      ...
      {/*
        ROL TEK DEĞERLİDİR ve açılır listedir. Yanında bir süre ÇOK SEÇİMLİ
        "Görev Etiketleri" çip kümesi durdu (Satın Alma · Planlama · Üretim);
        kullanıcı kararıyla (12.08.2026) o üçü de birer ROL oldu ve etiket
        sütunu kalktı. Yani bu hücre artık kişinin tek kimliğidir.
      */}
      <TableCell className="max-xl:p-0">
        <Label className="mb-1.5 block text-muted-foreground xl:hidden">Rol</Label>
        <Select value={role} onValueChange={(v) => setRole(v as UserRole)}>
          <SelectTrigger size="sm" className="w-full xl:w-44">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
            {USER_ROLES.map((r) => (
