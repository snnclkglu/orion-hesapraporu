...
import { hizmetGunu } from "@/lib/personnel/payroll";
import { expiryState } from "@/lib/personnel/employee";
import type {
  DocumentRow,
  EmployeeRow,
  EmploymentRow,
...
export async function loadEmployees(
  ...
  const [kisiler, donemler, belgeler] = await Promise.all([
    tumSatirlar<EmployeeDb>((bas, son) =>
      supabase.from("hr_employees").select(EMPLOYEE_SELECT).order("full_name").range(bas, son)
    ),
...
interface PayrollDb {
  ...
  cumulative_tax_base: number | string | null;
  params_valid_from: string | null;
}
...
function toPayroll(p: PayrollDb): PayrollRow {
  return {
    ...
    // gelebilir ve 0'a düşmek bordroyu "hiç çalışmamış" gösterirdi.
    workedDays: num(p.worked_days) || 30,
    cumulativeTaxBase: numOrNull(p.cumulative_tax_base),
    paramsValidFrom: p.params_valid_from,
  };
}
...
/**
 * Yasal bordro parametreleri — yıl yıl.
 *
 * Sütun eksikse (migration henüz uygulanmamışsa) sorgu TAMAMEN düşer ve bordro
 * ekranı açılmazdı; bu yüzden hata YUTULUR ve boş liste döner. Çekirdek
 * parametresiz dönemde hesap yapmaz, belge de bunu açıkça söyler.
 */
export async function loadPayrollParams(
  supabase: SupabaseClient
): Promise<PayrollParamsRow[]> {
  const { data, error } = await supabase
    .from("hr_payroll_params")
    .select(
      "valid_from, label, min_wage_gross, sgk_ceiling, sgk_employee_rate, unemployment_employee_rate, sgk_employer_rate, unemployment_employer_rate, stamp_tax_rate, income_tax_brackets, income_tax_exemption, stamp_tax_exemption, source, verified"
    )
    .order("valid_from", { ascending: false });
  if (error) return [];
  return (data ?? []).map((r) => {
    ...
    return {
      validFrom: String(p.valid_from ?? "").slice(0, 10),
      label: str(p.label as string),
      minWageGross: num(p.min_wage_gross as number),
      sgkCeiling: num(p.sgk_ceiling as number),
      sgkEmployeeRate: num(p.sgk_employee_rate as number),
      unemploymentEmployeeRate: num(p.unemployment_employee_rate as number),
      sgkEmployerRate: num(p.sgk_employer_rate as number),
      unemploymentEmployerRate: num(p.unemployment_employer_rate as number),
      stampTaxRate: num(p.stamp_tax_rate as number),
      brackets: Array.isArray(p.income_tax_brackets)
        ? (p.income_tax_brackets as { ust: number | null; oran: number }[])
        : [],
      incomeTaxExemption: num(p.income_tax_exemption as number),
      stampTaxExemption: num(p.stamp_tax_exemption as number),
      source: str(p.source as string),
...
/**
 * BİR AYIN SON YAYIN GÜNÜNÜN KURU — dönem kurunun kaynağı.
 *
 * Kullanıcı kararı (12.08.2026): kur kullanıcıdan SORULMAZ, ayın son günü
 * otomatik alınır. "Son gün" takvimin 31'i değil TCMB'nin o ay YAYIMLADIĞI
 * son gündür: ay hafta sonu ya da resmî tatille bitiyorsa o günün kuru
 * YOKTUR ve bir öncekine düşülür.
 */
export async function loadMonthEndRate(
  supabase: SupabaseClient,
  period: string
): Promise<{ date: string; eurTry: number; usdTry: number } | null> {
  const ay = period.slice(0, 7);
  const [y, m] = ay.split("-").map(Number);
  const sonGun = new Date(Date.UTC(y, m, 0)).toISOString().slice(0, 10);
  const { data, error } = await supabase
    .from("fx_rate_daily")
    .select("rate_date, eur_try, usd_try")
    .gte("rate_date", `${ay}-01`)
    .lte("rate_date", sonGun)
    .order("rate_date", { ascending: false })
    .limit(1)
    .maybeSingle();
  if (error || !data) return null;
  const r = data as { rate_date: string; eur_try: number | string; usd_try: number | string };
  return { date: r.rate_date, eurTry: num(r.eur_try), usdTry: num(r.usd_try) };
}
