...
import { documentSchema, type DocumentInput, type PersonnelActionResult } from "./schema";

// YOL KURALI SAF ÇEKİRDEKTEDİR (`lib/personnel/personnel.ts`) ve İKİ TARAF DA
 ...
 */
export async function registerDocument(input: DocumentInput): Promise<PersonnelActionResult> {
 ...
 * `can_see_personnel()`ten geçtiğini ve DENETİM İZİNE yazıldığını garanti eder —
...
export async function signDocumentUrl(
  ...
  await supabase.from("audit_log").insert({
    actor: user.id,
    action: "personnel.document.open",
