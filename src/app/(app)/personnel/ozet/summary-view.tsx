...
import { Banknote, CalendarClock, Euro, Timer, Users } from "lucide-react";
import {
  AYLIK_CALISMA_SAATI,
  donemOzeti,
  netCalismaSaati,
...
/**
 * ÖZET TABLOSUNDA ONDALIK YOKTUR (kullanıcı kararı, 12.08.2026).
 *
 * Tablo on iki sütunlu bir KARŞILAŞTIRMA ızgarasıdır; orada aranan şey
 * "hangi ay daha pahalıydı"dır, kuruş değil. "2.214.000" ile "2.214.000,00"
 * aynı bilgiyi taşır ama ikincisi sütunu yarı yarıya genişletir ve ekranı
 * kaydırtır. Kuruş gereken yer bordrodur ve orada tam basılır.
 */
...
function saatlikEur(a: AySatiri, kapsamli: (x: AySatiri) => ReturnType<typeof donemOzeti>) {
  ...
  const saat = netCalismaSaati(
    o.normalHours,
    o.overtimeHours,
    // İzin/rapor AY DÜZEYİNDEdir ve kapsama bölünemez; kapsam süzgeci
    // açıkken payda o kadar düşmez — sayı yine de karşılaştırılabilir kalır.
    a.leaveHours,
    a.reportHours
  );
    ...
    <div className="grid gap-3">
      ...
      <div className="grid grid-cols-2 gap-2 lg:grid-cols-3 xl:grid-cols-5">
        ...
        <StatCard
          ...
          value={sonAy ? `${tam(kapsamli(sonAy).overtimeHours)} saat` : "—"}
          hint={sonAy ? `${tam(kapsamli(sonAy).overtimeTotal)} ₺` : undefined}
          icon={Timer}
        />
        ...
        <StatCard
          ...
          value={
            sonAySaatlik?.eur !== null && sonAySaatlik !== null
              ? `${sonAySaatlik.eur!.toLocaleString("tr-TR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} €`
              : "—"
          }
          hint={
            sonAySaatlik
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className={cn(AT_MD, "text-right")}>Saatlik Maliyet (€)</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {aylar.map((a) => {
              ...
              const saatlik = saatlikEur(a, kapsamli);
              return (
                <TableRow key={a.period} className="hover:bg-muted/40">
                  ...
                  <TableCell className={cn(AT_XL, "text-right font-mono tabular-nums")}>
                    {o.overtimeHours > 0 ? tam(o.overtimeHourCost) : "—"}
                  </TableCell>
                  ...
                  <TableCell className={cn(AT_MD, "text-right font-mono tabular-nums")}>
                    {saatlik?.eur != null
                      ? saatlik.eur.toLocaleString("tr-TR", {
      ...
      <div className="grid gap-1 text-xs text-muted-foreground">
        ...
        {eksikKur > 0 && (
          <p className="text-amber-600 dark:text-amber-400">
            ...
            {canWrite && (
              <Link href="/personnel/maas" className="underline">
