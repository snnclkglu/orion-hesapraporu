...
import { canEditOffers, canEditReports, isAdminRole } from "@/lib/roles";
import {
  loadDrawingAuthors,
  loadDrawingPlan,
...
export async function ProjectPageView({
  ...
  const contextLabel = reportContextLabel(reportContext);
  ...
  const canWriteReports = offerContext
    ? canEditOffers(profile?.role)
    : canEditReports(profile?.role);
  // Teklif yöneticisi taslağı düzenleyebilir; kalıcı silme talebinin mevcut
  // veritabanı kapısı ise Mühendislik yazarı ya da Yönetici ister. Teklif
  // bağlamında düğmeyi yalnız Yöneticiye göstermek sessiz bir RLS hatasını
  // kullanıcıya yaşatmaz.
  const canDeleteRevision = offerContext ? isAdmin : canWriteReports;
  ...
  return (
    <div className="grid min-w-0 max-w-full gap-3 overflow-x-hidden sm:gap-4 lg:gap-6">
      ...
      <Tabs defaultValue="report">
                ...
                {revisionList.map((r) => (
                  <TableRow key={r.id}>
                    <TableCell data-label="Revizyon" data-mobile-revision data-mobile-hide-label className="font-mono">
                      <Link
                        href={`${basePath}/${project.id}/revisions/${r.id}`}
        ...
        {!offerContext && <TabsContent value="electrical">
          <ElectricalCard
            projectId={project.id}
        ...
        {!offerContext && <TabsContent value="manual">
          <ManualCard
            projectId={project.id}
            manual={elKitabi}
