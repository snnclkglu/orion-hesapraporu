...
// VERCEL HOBBY SINIRI: giriş, filigranlı içerik ve açık indirme ayrı route
// dosyalarına bölünmez. Her route dosyası ayrı serverless function sayılır;
// bu catch-all üç güvenlik ucunu tek fonksiyonda tutar (BELGE portal kuralı).
export const runtime = "nodejs";
export const dynamic = "force-dynamic";

type PortalRouteParams = Promise<{ code: string; action: string[] }>;
...
async function login(request: Request, code: string): Promise<Response> {
...
export async function GET(
  ...
  if (
    action.length !== 3 ||
    action[0] !== "belge" ||
    (action[2] !== "content" && action[2] !== "indir")
  ) {
    return new Response("Bulunamadı", { status: 404 });
  }
  return serveDocument(request, code, action[1], action[2]);
}
