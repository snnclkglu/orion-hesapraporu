...
export function AutoSelectionDialog({ revisionId, request, onApply, previewRows }: Props) {
  ...
  return <>
    <Dialog open={open} onOpenChange={value => { if (value) void show(); else { cancel(); setOpen(false); } }}>
      <DialogTrigger asChild><Button type="button" variant="outline" size="sm" className="oc-tap ml-auto gap-2"><WandSparkles className="size-4" />Hızlı otomatik seçim</Button></DialogTrigger>
      <DialogContent className="flex flex-col gap-0 overflow-hidden p-0 sm:gap-0 sm:p-0 sm:max-w-[min(72rem,calc(100%-2rem))]">
        <DialogHeader className="shrink-0 border-b bg-muted/30 px-5 py-4 pr-12"><DialogTitle className="text-lg">Hızlı otomatik seçim</DialogTitle><DialogDescription>Marka ve tasarım tercihlerini kontrol edin; seçilen ekipmanlar düzenlenebilir hesap taslağına uygulanır.</DialogDescription></DialogHeader>
        ...
        {loading || running ? <div className="space-y-3" role="status" aria-live="polite"><p className="flex items-center gap-2"><Loader2 className="size-4 animate-spin" />{progress?.stage ?? "Katalog hazırlanıyor"}</p><progress className="h-2 w-full" value={progress?.completed ?? 0} max={progress?.total || 1} /><Button variant="outline" className="oc-tap" onClick={cancel}>İptal et</Button></div> : result ? <div className="space-y-4">
          ...
          <section className="space-y-3" aria-label="Marka ve seri tercihleri"><div><h3 className="font-semibold">1. Marka ve seri tercihleri</h3><p className="mt-1 text-xs text-muted-foreground">Marka veya seri belirtmezseniz uygun katalog adayları birlikte değerlendirilir.</p></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{brandKeys.map(key => {
            const available = availableBrands(families ?? [], key, request);
            const types = availableSeries(families ?? [], key, brands[key] ?? "", request);
            return <div key={key} className="min-w-0 space-y-2 rounded-lg border bg-muted/20 p-3"><label className="block space-y-1 text-sm"><span className="font-medium">{BRAND_LABELS[key]}</span><select className="oc-tap w-full min-w-0 rounded-md border bg-background px-2.5 py-1.5 text-base pointer-fine:text-sm focus-visible:outline-2 focus-visible:outline-primary" value={brands[key] ?? ""} onChange={event => { setBrands({ ...brands, [key]: event.target.value }); setSeries({ ...series, [key]: defaultSeries(key, event.target.value) }); }}><option value="">Uygun markalar arasından seç</option>{brands[key] && !available.includes(brands[key]!) && <option value={brands[key]}>Bu ailede yok: {brands[key]}</option>}{available.map(brand => <option key={brand}>{brand}</option>)}</select></label>
              {SERIES_KEYS.includes(key) && <label className="block space-y-1 text-sm"><span className="text-xs text-muted-foreground"><span className="sr-only">{BRAND_LABELS[key]} · </span>Seri / tip</span><select className="oc-tap w-full min-w-0 rounded-md border bg-background px-2.5 py-1.5 text-base pointer-fine:text-sm focus-visible:outline-2 focus-visible:outline-primary" value={series[key] ?? ""} onChange={event => setSeries({ ...series, [key]: event.target.value })}><option value="">Uygun seriler arasından seç</option>{series[key] && !types.includes(series[key]!) && <option value={series[key]}>Bu ailede yok: {series[key]}</option>}{types.map(type => <option key={type} value={type}>{type}</option>)}</select></label>}
            </div>;
          })}</div></section>
          ...
          <section className="space-y-3 rounded-lg border p-4"><h3 className="font-semibold">3. Hesap seçenekleri</h3>
          ...
          <details className="rounded-lg border p-3"><summary className="oc-tap cursor-pointer text-sm font-medium">Korunacak bölümler</summary><div className="grid gap-2 pt-2 sm:grid-cols-2">{request.active.map(key => <label key={key} className="oc-tap flex items-center gap-2 text-sm"><input type="checkbox" checked={locks.includes(key)} onChange={event => setLocks(event.target.checked ? [...locks, key] : locks.filter(lock => lock !== key))} />{MODULE_LABELS[key]} seçimlerini koru</label>)}</div></details>

          <details className="rounded-lg border p-3"><summary className="oc-tap cursor-pointer text-sm font-medium">Ekipman ve ölçü kilitleri</summary>          <p className="text-sm text-muted-foreground">Ekipman ve ölçü kilidi: işaretlediğiniz mevcut seçimi otomatik işlem değiştirmez. Örneğin motoru kilitlerseniz motor korunur, diğer ekipmanlar ona göre aranır. İlk hesapta boş bırakabilirsiniz; kilit uygunluk kontrolünü kaldırmaz.</p><div className="max-h-60 space-y-3 overflow-y-auto pt-2">{request.active.filter(key => isHoistKey(key) || isTravelKey(key)).map(key => {
          ...
          <details className="rounded-lg border p-3"><summary className="oc-tap cursor-pointer text-sm font-medium">Hesap yöntemi ve toleranslar</summary><div className="space-y-2 pt-3">          <p className="text-sm text-muted-foreground">Hesap sırası: halat → tambur ve miller → motor/redüktör → fren/kaplin → kanca/makara → yürütme → taşıyıcı yapı ve son kontroller. Motor güçleri, çaplar ve katalog büyüklükleri bu sırada hesaplanır; bağlı seçim gerektiğinde birlikte yeniden denenir.</p><p className="text-xs text-muted-foreground">Motor: standart 1500 dev/dak sınıfı, yakın katalog devirleri ±%10 kabul edilir. Gerçek hareket hızı: ±%5 firma toleransı. Seçimden sonra hesap ve bağlı ekipmanlar otomatik denetlenir; gerektiğinde en fazla üç düzeltme turu yapılır. Verdiğiniz donanım, teker/tahrik sayıları, raylar ve kilitler korunur.</p>
          </div></details>
          {error && <Button variant="outline" className="oc-tap" onClick={show}>Kataloğu tekrar yükle</Button>}
        </>}
        </div>
        {!loading && !running && !result && <div className="flex shrink-0 flex-col gap-3 border-t bg-muted/30 px-4 py-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] sm:flex-row sm:items-center sm:justify-between sm:px-5"><p className="text-xs text-muted-foreground">Sonuç taslağa uygulanır. Kaydetmeden önce raporu inceleyebilirsiniz.</p><Button className="oc-tap shrink-0 gap-2" disabled={!families} onClick={start}><WandSparkles className="size-4" />Hesap raporunu oluştur</Button></div>}
      </DialogContent>
    </Dialog>
  </>;
}
