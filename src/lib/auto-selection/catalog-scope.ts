...
export const catalogFilterSchema = z.array(z.object({ kind: z.enum(CATALOG_KINDS), brands: z.array(z.string().min(1).max(200)).max(100).nullable(), application: z.enum(["kaldirma", "yurutme"]).optional() })).max(20);
export type CatalogFilter = z.infer<typeof catalogFilterSchema>;
export function selectionCatalogFilter(request: Pick<SelectionRequest, "active" | "brands">): CatalogFilter {
  ...
  return CATALOG_KINDS.filter(kind => kinds.has(kind)).flatMap((kind): CatalogFilter => {
    if (kind === "gearbox") return [
      ...(request.active.some(isHoistKey) ? [{ kind, brands: request.brands.hoistGearbox ? [request.brands.hoistGearbox] : null, application: "kaldirma" as const }] : []),
      ...(request.active.some(isTravelKey) ? [{ kind, brands: request.brands.travelGearbox ? [request.brands.travelGearbox] : null, application: "yurutme" as const }] : []),
    ];
