...
export function pushChartFrame(
  ...
  if (opts?.title) {
    els.push(txt(x - 44, y - 26, opts.title, 10.5, { fill: DCOL.accent, bold: true }));
    if (opts.note) els.push(txt(x - 44, y - 14, opts.note, 8, { fill: DCOL.muted }));
  }
  ...
  // Tikler ve etiketleri
  for (const t of xt) {
    const tx = px(t);
    els.push(ln(tx, y + h, tx, y + h + 4, DCOL.ink, 0.8));
    els.push(txt(tx, y + h + 14, fx(t), 8, { anchor: "middle", fill: DCOL.muted }));
  }
  for (const t of yt) {
    const ty = py(t);
    els.push(ln(x - 4, ty, x, ty, DCOL.ink, 0.8));
    els.push(txt(x - 7, ty + 3, fy(t), 8, { anchor: "end", fill: DCOL.muted }));
  }

  // Eksen başlıkları
  els.push(txt(x + w + 14, y + h + 14, ax.label, 9, { anchor: "end", fill: DCOL.ink }));
  els.push(txt(x - 2, y - 18, ay.label, 9, { anchor: "start", fill: DCOL.ink }));
...
export function pushVGuide(
  ...
  if (label) {
    ...
    els.push(txt(cx, y, label, 8, { anchor: "middle", fill: c }));
  }
}
...
export function pushHGuide(
  ...
  if (label) {
    ...
    els.push(
      txt(anchor === "end" ? f.area.x + f.area.w - 4 : f.area.x + 4, cy - 4, label, 8, {
        anchor, fill: c,
      })
    );
  }
...
export function pushUtilizationBar(
  ...
  els.push({ kind: "rect", x, y, w, h, rx: 3, fill: "#FFFFFF", stroke: DCOL.line, strokeWidth: 1 });
  els.push({
  ...
  els.push(txt(x + w, y - 6, "%100", 7.5, { anchor: "middle", fill: DCOL.ink }));
  if (opts?.label) els.push(txt(x, y - 6, opts.label, 8.5, { fill: DCOL.ink }));
  els.push(
