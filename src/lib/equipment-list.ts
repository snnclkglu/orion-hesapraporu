...
//
// Dosya `lib/excel/equipment.ts`ten AYRILDI ve gövdeleri değişmeden taşındı.
// Sebep ölçülebilir: o dosya `exceljs`i tepeden içe aktarıyor ve revizyon
// editörü (istemci) ekipman satırlarını okumak isteyince koca bir Excel
// kütüphanesi mühendisin gününü geçirdiği ekranın paketine giriyordu. Excel
// YAZIMI orada kaldı; burada yalnız "bu vinçte hangi ekipman var, kaç adet,
// hangisi gizli" sorusunun cevabı durur. `lib/excel/equipment.ts` bu API'yi
...
import { COUPLING_SEAL_TYPE_STANDARD, COUPLING_WEAR_DETECTION_STANDARD } from "@/lib/calc/fields";
...
import { din15407Row, hookStandardOf, isLamellaHook } from "@/lib/calc/hook-standards";
import { hookCapacityKg } from "@/lib/calc/hook-table";
import { drumBrakeSpec } from "@/lib/calc/drum-brake";
import { safetyBrakeByCode } from "@/lib/calc/safety-brake";
import { wedgeSocketForRope } from "@/lib/calc/wedge-socket";
...
/**
 * Boş/null metin yerine "-" — Excel yazıcısı da aynı boşluk dilini kullandığı
 * için DIŞA AÇIKTIR (iki dosya iki farklı boşluk işareti basmamalı).
 */
export const textOr = (s: string | null | undefined, fallback = "-"): string =>
...
export interface EqRow {
  ...
  qty: number | string;
  /**
   * SATIR BAŞINA ÜRÜN AĞIRLIĞI [kg] — adetle ÇARPILMAMIŞ hâli.
   *
   * Tek okuyucusu AĞIRLIK DÖKÜMÜ penceresidir; ekipman listesi bu alanı BASMAZ
   * (ekran, Excel ve PDF sütun düzeni değişmedi) ve hesap motoru hiç okumaz.
   *
   * `undefined` = BİLİNMİYOR, `0` DEĞİL (değişmez md. 4): katalogda ağırlık
   * yayımlanmamış olabilir (rulman yatağı · klima · feston) ya da ürün bu alan
   * eklenmeden önceki bir revizyonda seçilmiş olabilir. Döküm "—" basar.
   */
  weightKg?: number;
  /**
   * Katalog ağırlığı ARALIK verdiyse ÜST sınır (Jaure kaplinleri, Eldro itici).
...
function hoistRows(
  ...
  return [
    ...
    // TAMBUR EMNİYET FRENİ (2.8) — uzun süre HİÇ SATIR ÜRETMİYORDU.
    //
    // Seçilen SIBRE SHI kaliperi ne ekipman listesinde ne ağırlık dökümünde
    // görünüyordu; satın alma da onu görmüyordu. Satır yalnız frenin OLDUĞU
    // kaldırma grubunda ve MODEL SEÇİLMİŞSE çıkar — "seçim yapılmadı" bir
    // teknik özellik değildir ve satır açmaz.
    ...(hasSafetyBrake(specs, moduleKey) && (sel.safetyBrakeModel ?? "").trim()
      ? [safetyBrakeRow(rk("safetyBrake"), sel)]
      : []),
    ...balanceRows(moduleKey, inp, sel, specs),
  ];
}

/** 2.8 kaliper satırı — ölçüler seçimden, min disk kalınlığı defterden. */
function safetyBrakeRow(rowKey: string, sel: HoistSelections): EqRow {
  const kaliper = safetyBrakeByCode(sel.safetyBrakeModel);
  const unite = (sel.safetyBrakeHydraulicUnit ?? "").trim();
  return {
    rowKey,
    kind: "brake",
    component: "Tambur emniyet freni",
    brand: "SIBRE",
    model: textOr(sel.safetyBrakeModel),
    spec:
      `hava aralığı ${fmt(sel.safetyBrakeAirGapMm, 1)} mm, ` +
      `flanş Ø${fmt(sel.safetyBrakeFlangeDiaMm)} × ${fmt(sel.safetyBrakeFlangeThicknessMm)} mm` +
      (kaliper ? `, min disk ${fmt(kaliper.minDiscThicknessMm)} mm` : "") +
      (unite ? `, ünite ${unite}` : ""),
    qty: 1,
  };
}
 ...
 */
/** Pozitif ve sonlu bir sayıysa kendisi, değilse `undefined` (0 asla ağırlık değildir). */
function agirlikVeyaYok(v: unknown): number | undefined {
  return typeof v === "number" && Number.isFinite(v) && v > 0 ? v : undefined;
}

type AgirlikCozumu = { kg?: number; ust?: number } | undefined;

/**
 * SATIR BAŞINA AĞIRLIK — tek çözücü, satır gövdelerinin DIŞINDA.
 *
 * Ağırlık, yirmi beş satır nesnesinin içine serpiştirilmek yerine burada
 * MERKEZÎ olarak eklenir: satır üreticileri "hangi ekipman var, kaç adet"
 * sorusunun cevabıdır ve onların okunurluğunu ikinci bir soruyla bozmak
 * gereksizdi. Anahtar `rowSlug`tur — gizleme süzgeci, notlar ve ekler de aynı
 * slug uzayını kullanır, yani ağırlık kendiliğinden doğru satıra oturur.
 *
 * Alternatif (seçenekli) satırlar da KENDİ ağırlığını alır: `withAlternativeRows`
 * `moduleEquipmentRows`u seçenek seçimleriyle yeniden çağırır ve bu çözücü de o
 * çağrının içindedir.
 */
function agirlikCozucu(
  key: ModuleKey,
  inputs: object,
  selections: object,
  specs: TechnicalSpecs
): ((slug: string) => AgirlikCozumu) | null {
  if (isHoistKey(key)) {
    const inp = inputs as HoistInputs;
    const sel = selections as HoistSelections;
    const ropePlan = ropeLengthPlan(inp, sel, hoistSpecView(specs, key).liftHeightM);
    const halatKg = (lay: "right" | "left"): AgirlikCozumu => {
      const kgm = agirlikVeyaYok(sel.ropeWeightKgPerM);
      const boy = agirlikVeyaYok(ropePlan.lines.find((l) => l.lay === lay)?.lengthPerPieceM);
      return kgm !== undefined && boy !== undefined ? { kg: kgm * boy } : undefined;
    };
    // KASNAK FRENİNDE AĞIRLIK FREN + İTİCİDİR (HESAP-27). Katalogun kg* sütunu
    // İTİCİ HARİÇTİR; doğru sayı `drum-brake.ts` defterinin toplamıdır ve
    // katalog aralık verdiğinde üst sınır da korunur.
    const frenDefteri = drumBrakeSpec(sel.brakeModel);
    const soket = wedgeSocketForRope(sel.ropeDiaMm, sel.balanceSocketType);
    return (slug) => {
      switch (slug) {
        case "rope": return halatKg("right");
        case "ropeLeft": return halatKg("left");
        case "drum": return { kg: agirlikVeyaYok(inp.drumWeightKg) };
        case "drumBearing": return { kg: agirlikVeyaYok(sel.bearingWeightKg) };
        case "gearbox": return { kg: agirlikVeyaYok(sel.gearboxWeightKg) };
        case "motor": return { kg: agirlikVeyaYok(sel.motorWeightKg) };
        case "brake":
          return frenDefteri
            ? { kg: frenDefteri.totalWeightKg, ust: frenDefteri.totalWeightMaxKg }
            : { kg: agirlikVeyaYok(sel.brakeWeightKg) };
        case "motorCoupling":
          return {
            kg: agirlikVeyaYok(sel.motorCouplingWeightKg),
            ust: agirlikVeyaYok(sel.motorCouplingWeightMaxKg),
          };
        case "drumCoupling":
          return {
            kg: agirlikVeyaYok(sel.drumCouplingWeightKg),
            ust: agirlikVeyaYok(sel.drumCouplingWeightMaxKg),
          };
        case "safetyBrake":
          return { kg: agirlikVeyaYok(safetyBrakeByCode(sel.safetyBrakeModel)?.weightKg) };
        case "balanceSocket": return { kg: agirlikVeyaYok(soket?.weightKg) };
        case "balanceBearing": return { kg: agirlikVeyaYok(sel.balanceBearingWeightKg) };
        default: return undefined;
      }
    };
  }
  if (isHookBlockKey(key)) {
    const sel = selections as HookBlockSelections;
    return (slug) => {
      switch (slug) {
        case "hook": return { kg: agirlikVeyaYok(sel.hookWeightKg) };
        case "sheave": return { kg: agirlikVeyaYok(sel.sheaveWeightKg) };
        case "sheaveBearing": return { kg: agirlikVeyaYok(sel.sheaveBearingWeightKg) };
        case "hookBearing": return { kg: agirlikVeyaYok(sel.hookBearingWeightKg) };
        default: return undefined;
      }
    };
  }
  if (isTravelKey(key)) {
    const sel = selections as TravelSelections;
    // Yürütme freninin kimliği TEK BİRLEŞİK alandadır ("MARKA MODEL"); defter
    // kod ayrıştırmasını kendisi yapar ve tanımadığı kodda `null` döner.
    const frenDefteri = drumBrakeSpec(sel.brakeBrand);
    return (slug) => {
      switch (slug) {
        case "wheel": return { kg: agirlikVeyaYok(sel.wheelWeightKg) };
        case "wheelBearing": return { kg: agirlikVeyaYok(sel.bearingWeightKg) };
        case "motor": return { kg: agirlikVeyaYok(sel.motorWeightKg) };
        case "gearbox": return { kg: agirlikVeyaYok(sel.gearboxWeightKg) };
        case "brake":
          return frenDefteri
        ...
        case "wheelCoupling":
          return {
            kg: agirlikVeyaYok(sel.wheelCouplingWeightKg),
            ust: agirlikVeyaYok(sel.wheelCouplingWeightMaxKg),
          };
        case "buffer": return { kg: agirlikVeyaYok(sel.bufferWeightKg) };
        default: return undefined;
      }
    };
  }
...
/** Ham satırlara ağırlığı ekler; slug tanınmıyorsa satır AYNEN geçer. */
function satirAgirliklariniUygula(
  key: ModuleKey,
  rows: EqRow[],
  inputs: object,
  selections: object,
  ...
  const coz = agirlikCozucu(key, inputs, selections, specs);
  if (!coz) return rows;
  return rows.map((row) => {
    const slug = rowSlug(row.rowKey, key);
    if (!slug) return row;
    const bulunan = coz(slug);
    if (!bulunan || bulunan.kg === undefined) return row;
    return {
      ...row,
      weightKg: bulunan.kg,
      ...(bulunan.ust !== undefined ? { weightKgUst: bulunan.ust } : {}),
    };
  });
}
...
function moduleEquipmentRows(
  ...
  const rows = moduleEquipmentRowsHam(key, inputs, selections, specs, cabinValues);
  return rows === null ? null : satirAgirliklariniUygula(key, rows, inputs, selections, specs);
}

function moduleEquipmentRowsHam(
  key: ModuleKey,
  inputs: object,
  selections: object,
  specs: TechnicalSpecs,
 ...
 */
export function hiddenEquipmentSlugs(key: ModuleKey, hidden: ReadonlySet<string>): Set<string> {
  const out = new Set<string>();
 ...
 */
export function rowSlug(rowKey: string | undefined, key: ModuleKey): string | undefined {
