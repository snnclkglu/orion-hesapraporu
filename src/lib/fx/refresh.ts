...
//      tabanlı istemci, RLS `can_edit_personnel()`ten geçer).
...
import { eksikGunAraligi } from "./rates";
import { cekGunlukKurlar } from "./source";
...
export async function tazeleKurlar(
  ...
  if (sonuc.rows.length > 0) {
    ...
    const { error, count } = await supabase.from("fx_rate_daily").upsert(
      sonuc.rows.map((r) => ({
        rate_date: r.date,
        source: r.source,
        usd_try: r.usdTry,
