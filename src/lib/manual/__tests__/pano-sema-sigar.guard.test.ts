...
// görünüşü, tam genişlikte ~666 pt. Gövde yüksekliği ~700 pt; pay dardır ve
// bu yüzden ölçülür.
...
describe("pano şeması el kitabı gövdesine sığar", () => {
  it("fikstür gerçekten en kötü durumu kuruyor", () => {
    ...
    // Kapak görünüşü de üretilmeli; en uzun şema odur.
    expect(ENKOTU.room[0].doorPlacements.length).toBeGreaterThan(0);
  });

  it("TAM GENİŞLİKTE bazı şemalar sayfayı TAŞIRIR — sorun gerçektir", () => {
    // Bu testin var oluş sebebi: 2.000 mm'lik dar bir panonun kapak görünüşü
    // tam genişlikte gövdeyi AŞIYOR. Sorunu kanıtlamadan çözümü çivilemek,
    // gereksiz bir kural bırakmak olurdu.
    const tasan = panoSemaKatalogu(ENKOTU).filter((x) => {
      const d = x.ciz();
      return d ? (TAM_GENISLIK * d.height) / d.width > MANUAL_GOVDE_YUKSEKLIK : false;
    });
    expect(tasan.length).toBeGreaterThan(0);
  });
  ...
  it("payın ne kadar olduğu GÖRÜNÜR kalır", () => {
    ...
    // 08.09.2026'da ölçülen: en uzun şema (kapak görünüşü) %95'e iniyor.
    expect(enDar).toBeGreaterThanOrEqual(80);
    expect(enDar).toBeLessThan(100);
  });
});
