...
export interface ManualRenderOptions {
  payloadOverride?: ManualPayload;
  strict?: boolean;
  onManifest?: (manifest: Record<string, unknown>) => void;
}

/** Yetki kontrolünden geçmiş çağıran için ortak PDF üretimi; hiçbir kayıt yazmaz. */
export async function renderManualResponse(
  supabase: SupabaseClient,
  id: string,
  revId: string,
  eklerIstendi: boolean,
  options: ManualRenderOptions = {},
) {
  ...
  if (revizyon.row.status === "issued" && !options.payloadOverride) {
    const { data: record, error } = await supabase.from("manual_revisions").select("delivery_archive").eq("id", revId).single();
    if (error && revizyon.payload.designVersion === 2) return new Response("Teslim arşivi okunamadı.", { status: 503 });
    const archive = record?.delivery_archive;
    if (archive) {
      const file = archive[eklerIstendi ? "full" : "body"];
      const { data, error: downloadError } = await supabase.storage.from("manual-deliveries").download(file.path);
      if (downloadError || !data) return new Response("Arşiv dosyası alınamadı; yeniden üretim yapılmadı.", { status: 503 });
      const bytes = new Uint8Array(await data.arrayBuffer());
      if (createHash("sha256").update(bytes).digest("hex") !== file.sha256) return new Response("Arşiv bütünlük kontrolü başarısız.", { status: 503 });
      return new Response(bytes, { headers: { "Content-Type": "application/pdf", "Content-Disposition": `attachment; filename="manual-${revId}-${eklerIstendi ? "full" : "body"}.pdf"`, "X-Manual-Archive": "1" } });
    }
    if (revizyon.payload.designVersion === 2) return new Response("Bu yayının teslim arşivi eksik.", { status: 409 });
  }
  if (options.payloadOverride) revizyon.payload = options.payloadOverride;
  ...
  if (options.strict) {
    const issues = manualContentIssues(payload, new Set(gorseller.map(g => g.id)));
    if (issues.length) return new Response(`Yayım kalite kapısı: ${issues.map(i => i.message).join("; ")}`, { status: 422 });
  }
  ...
  if (options.strict && oncedenAtlanan.length) return new Response(`Seçili ekler eksik: ${oncedenAtlanan.join("; ")}`, { status: 422 });
  ...
  const govde = await renderToBuffer(
    ManualPdf({
      ...
      bandLines: [
        ...
        ...(payload.designVersion === 2 && oncedenAtlanan.length ? [`EKSİK EK VAR (${oncedenAtlanan.length}) · TESLİM ÖNCESİ KONTROL`] : []),
      ],
      appendixPageCounts,
  ...
  if (options.strict && atlanan) return new Response(`Teslim birleştirilemedi: ${atlananAd}`, { status: 422 });
  options.onManifest?.({ designVersion: payload.designVersion ?? 1, sources: kaynaklar, project: proje, appendixPageCounts,
    appendices: cozulmusEkler.map(e => ({ kind: e.tur, pages: e.pageCount, sha256: createHash("sha256").update(e.bytes).digest("hex") })),
    images: gorseller.map(g => ({ id: g.id, sha256: createHash("sha256").update(g.bytes).digest("hex") })), omitted: atlananAd });
