...
import { buildNameplateSvg, productPortalUrl } from "../nameplate";
...
describe("baskı isim plakası", () => {
  it("mm ölçüsünü, kalıcı portal URL'sini ve Q hata düzeltmeli QR geometrisini üretir", () => {
    const url = productPortalUrl("https://orioncranes.com/", "23456789ABCDEFGH");
    ...
    expect(url).toBe("https://orioncranes.com/paylas/vinc/23456789ABCDEFGH");
    expect(svg).toContain('width="240mm" height="160mm"');
    expect(svg).toContain("0057-01-A");
    expect(svg).toContain('shape-rendering="crispEdges"');
    expect(svg).toContain('fill="#000000"');
    expect(svg).not.toContain("#A41E1E");
  });
